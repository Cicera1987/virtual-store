...
import { StyleInput, ContaineItemLi } from "./style"
import { ContaineItem} from '../ModalCreate/style';
    ...
    return (
        <>
            <StyleForm action='#' onSubmit={handleSubmit}>
                <StyleInput type="text" onChange={changeHandler} />
                <span>
                    <ButtonLogin onClick={handleSubmission}>Produto</ButtonLogin>
                </span>
            </StyleForm>
            <ContaineItem>
                {produtos.map( person => {
                    return (
                    <ContaineItemLi key={person.name}>
                        <img 
                        width="100%"
                        height="380px"
                        src={person.img} 
                        alt="produto" />
                        <p>Produto: {person.name}</p>
                        <p>Codigo:{person.id}</p>
                        <p>Valor: {person.price}</p>
                    </ContaineItemLi>
                    );
                    })}
            </ContaineItem>
